...
let bonusstars, mainskin, geneValue, imgability
let gene1, gene2
    ...
    .then((data) => {
      ...
      Papa.parse(data, {
        complete: function (results) {
          ...
          const MNameLevel = document.getElementById('mnamelevel')
          ...
          if (resultRow) {
            ...
            extractedData = {
              ...
              imgability: resultRow[15]?.trim(), //image ability
              ability: resultRow[17]?.trim(), //abilitp
            }
            ...
            /*resultsDiv.innerHTML = `
              ...
              `*/
            MPic.innerHTML = `
            <p><img src="${imagepath}" alt="Specimen Image" /></p>
            `
            Mtype.innerHTML = `
            <p><img src="${iconLink}" alt="Type Icon" /></p>
            `
            MNameLevel.innerHTML = `
            <p><strong>Name:</strong> ${extractedData.name}</p>
            <p><strong>Level:</strong> ${level}</p>
            `
            ...
            getAbilityImage(extractedData.imgability)
            calcatkp()
            calcspeed()
...
function calcatkp() {
  if (extractedData.atk1p && extractedData.atk2p) {
    ...
    let abilityImageUrl = getAbilityImage(extractedData.imgability); // Obtener la URL de la imagen

    document.getElementById('calculatedatk').innerHTML = `
      <div>
        <img src="${genes.gene1}" alt="Gene1" width="30"> 
        <span>${stats.atk1p.toFixed(0)}</span>
        <br>
        <span><img src="${abilityImageUrl}" alt="Ability" width="30"> : ${stats.ability}</span>
        <br>
        <img src="${genes.gene2}" alt="Gene2" width="30"> 
        <span>${stats.atk2p.toFixed(0)}</span>
      </div>
`
  } else {
...
function getAbilityImage(abilityName) {
  let match = abilityName.match(/ability_([a-zA-Z]+)/);
  let extractedAbility = match ? match[1] : null;
  
  if (extractedAbility && ability.hasOwnProperty(extractedAbility)) {
    return urlabi + ability[extractedAbility]; // Retorna la URL de la imagen
  } else {
    console.log("Ability not found");
    return ''; // Retorna un string vacío si no se encuentra
  }
}
        ...
        complete: function(results) {
          ...
          for (let i = 1; i < rows.length; i++) {
            if (rows[i][0].includes(`Specimen_${specimen}`)) {
              const optionName = rows[i][4]?.trim(); // Suponiendo que la versión esté en la columna 3
              if (optionName) {
